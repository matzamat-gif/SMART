...
2. קרא את השלטים והתוויות בתמונה. בדוכני "נוי השדה" מעל/לפני כל ארגז יש שלט מחיר שלרוב כולל את שם המוצר בעברית. השם הכתוב על השלט הוא הראיה החזקה ביותר לזיהוי — חזק יותר מניחוש לפי צורה. שייך כל שלט לארגז הסמוך לו (השלט יושב לפני/מעל התוצרת שלו). אם שם קריא על שלט תואם לתוצרת שמתחתיו — השתמש בו, וגם עבור זוגות דומים מותר להעלות את הביטחון עד 0.85. אם השלט מטושטש/חתוך/לא קריא — התעלם ממנו וזהה חזותית.
...
4. ספור בשיטתיות: קודם יחידות גלויות (שורות × עמודות), ואז הוסף הערכת שכבות מוסתרות לפי עומק הכלי או הערימה. דוגמה: 8 עגבניות גלויות בקערה בעומק של ~2 שכבות → ~16 יחידות. אל תדווח רק את מה שנראה ואל תנפח מעבר לסביר.
5. משקל: למוצר ספיר — count × משקל ליחידה. אם המוצר בקטלוג, השתמש במשקל הקטלוגי (הוא מכויל), אלא אם היחידות בתמונה חריגות בגודלן. לתפזורת — הערך totalWeightGrams לפי נפח.
